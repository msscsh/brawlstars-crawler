...
const usedBrawlerGroups: Brawler[] = [];
...
function generateUniqueQuestion(): Question {
	...
	do {
		...
		isCombinationUsed(question.answer, question.type, question.attribute)
	);
...
function generateRandomQuestion(): Question {
	...
	if (randomType === "attribute") {
		return generateAttributeQuestion(
			getRandomOption(brawlersValidAttributesList),
		);
	} else {
		return generateDirectInfoQuestion(
			getRandomOption(brawlersDirectValuesList),
		);
	}
}
...
function generateAttributeQuestion(attribute): Question {
	...
	const answer = selectedBrawlers.reduce((a, b) =>
		a[attribute] > b[attribute] ? a : b,
	).nome;
	const options = gerarOpcoesComValoresDosAtributosDosBrawlers(
		selectedBrawlers,
		"nome",
		answer,
	);
...
function gerarOpcoesComValoresDosAtributosDosBrawlers(
	brawlersSelecionados,
	attribute,
	answer,
) {
...
function inicializarGlobais() {
	...
	brawlersValidAttributesList = getBrawlerAttributes(brawlers[0]).filter(
		(item) => item === "vida" || item === "Dano máximo por munição",
	);

	brawlersDirectValuesList = getBrawlerAttributes(brawlers[0]).filter(
		(item) =>
